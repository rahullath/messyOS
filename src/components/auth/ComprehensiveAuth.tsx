...
function AuthInterface() {
  ...
  useEffect(() => {
    const handleUserSync = async () => {
      if (authenticated && user && ready && !syncExecuted.current) {
        syncExecuted.current = true; // Prevent multiple executions
        ...
        try {
          ...
          syncExecuted.current = false; // Allow retry on error
        } finally {
          setIsLoading(false);
        }
      }
...
export default function ComprehensiveAuth() {
  ...
  return (
    <PrivyProvider
      appId={appId}
      config={{
        // Login methods - enable Google OAuth

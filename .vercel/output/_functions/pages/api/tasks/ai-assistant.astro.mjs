...
const POST = async ({ request, cookies }) => {
  try {
    ...
    const [tasksResult, sessionsResult, habitsResult] = await Promise.all([
      ...
      supabase.from("task_sessions").select("*").eq("user_id", user.id).gte("started_at", new Date(Date.now() - 30 * 24 * 60 * 60 * 1e3).toISOString()).order("started_at", { ascending: false }),
      supabase.from("habits").select("name, streak_count, category").eq("user_id", user.id).eq("is_active", true)
    ]);
...
function generateFallbackAnalysis(tasks, sessions, type) {
  const completedTasks = tasks.filter((t) => t.status === "completed");
  const incompleteTasks = tasks.filter((t) => t.status !== "completed");
  const avgSessionDuration = sessions.length > 0 ? sessions.reduce((sum, s) => sum + (s.duration || 0), 0) / sessions.length / 60 : 0;
  switch (type) {
    case "productivity_analysis":
      return {
        insights: [
          {
            ...
            recommendation: completedTasks.length / tasks.length > 0.7 ? "Great completion rate! Keep maintaining this momentum." : "Focus on completing smaller tasks first to build momentum.",
            confidence: 0.9
          },
          {
            ...
            recommendation: avgSessionDuration < 25 ? "Try longer focused sessions (25-45 minutes) for better deep work." : "Good session length! Consider the Pomodoro technique for optimization.",
            confidence: 0.8
          }
        ],
        optimizations: [

...
const GET = async ({ url, cookies }) => {
  ...
  try {
    ...
    const { data: healthMetrics, error } = await supabase.from("metrics").select("type, value, unit, metadata, recorded_at").eq("user_id", user.id).in("type", [
      "sleep_duration",
      "heart_rate_avg",
      "heart_rate_min",
      "heart_rate_max",
      "stress_level",
      "health_score",
      "medication_bupropion_morning",
      "medication_bupropion_afternoon",
      "medication_melatonin_evening"
    ]).gte("recorded_at", startDate.toISOString()).order("recorded_at", { ascending: false });
    ...
    const heartRateVariability = heartRateData.length > 1 ? Math.round(Math.sqrt(heartRateData.reduce((sum, m, i, arr) => {
      if (i === 0) return 0;
      const diff = m.value - arr[i - 1].value;
      return sum + diff * diff;
    }, 0) / (heartRateData.length - 1))) : 0;
    ...
    const recentMedication = medicationData.filter(
      (m) => new Date(m.recorded_at) >= last30Days
    );
    const bupropionEntries = recentMedication.filter(
      (m) => m.type.includes("bupropion")
    );
    const melatoninEntries = recentMedication.filter(
      (m) => m.type.includes("melatonin")
    );
    const bupropionTaken = bupropionEntries.filter((m) => m.value === 1).length;
    const bupropionTotal = Math.max(bupropionEntries.length, 60);
    ...
    const melatoninTotal = Math.max(melatoninEntries.length, 30);

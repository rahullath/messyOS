...
function calculateContentStats(content) {
  ...
  const avgRating = ratedContent.length > 0 ? ratedContent.reduce((sum, item) => {
    let rating = 0;
    if (typeof item.Rating === "string" && item.Rating.includes("/")) {
      rating = parseInt(item.Rating.split("/")[0]) / 20;
  ...
  const topGenres = Array.from(genreCount.entries()).map(([genre, count]) => ({ genre, count })).sort((a, b) => b.count - a.count).slice(0, 8);
  ...
  const upcomingEpisodes = tvShows.filter((show) => show.Status === "Returning Series").slice(0, 8).map((show) => ({
    title: show.TMDB_Title || show.Title,
    season_episode: "Next Episode",
    air_date: "TBA",
    network: show.Networks || "Unknown",
    poster_url: show.TMDB_ID ? `https://image.tmdb.org/t/p/w300${show.poster_path || ""}` : void 0
  }));
  const recentlyWatched = content.filter((item) => item.Watch_Date).sort((a, b) => new Date(b.Watch_Date).getTime() - new Date(a.Watch_Date).getTime()).slice(0, 10);
  return {
...
function getLanguageStats(content) {
  ...
  return Array.from(languageCount.entries()).map(([language, count]) => ({ language, count })).sort((a, b) => b.count - a.count).slice(0, 5);
}
function getRatingDistribution(content) {
...
function getPlatformStats(content) {
  ...
  return Array.from(platformCount.entries()).map(([platform, count]) => ({ platform, count })).sort((a, b) => b.count - a.count).slice(0, 8);
}
async function generateRecommendations(content) {

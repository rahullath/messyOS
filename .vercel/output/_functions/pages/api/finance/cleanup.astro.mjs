  ...
  try {
    ...
    switch (action) {
      ...
      case "clean-large-amounts":
        result = await cleanLargeAmounts(supabase, user.id, options?.threshold || 2e4);
        break;
      case "analyze-data":
...
async function cleanAllFinanceData(supabase, userId) {
  console.log("🧹 Cleaning ALL finance data...");
  const { data: beforeCount } = await supabase.from("metrics").select("id", { count: "exact", head: true }).eq("user_id", userId).in("type", ["expense", "income", "crypto_value"]);
  const { error } = await supabase.from("metrics").delete().eq("user_id", userId).in("type", ["expense", "income", "crypto_value"]);
...
async function cleanDuplicates(supabase, userId) {
  console.log("🔍 Finding and removing duplicates...");
  const { data: metrics } = await supabase.from("metrics").select("*").eq("user_id", userId).in("type", ["expense", "income"]);
...
async function cleanInternalTransfers(supabase, userId) {
...
async function cleanLargeAmounts(supabase, userId, threshold = 2e4) {
  console.log(`💰 Removing transactions over ₹${threshold.toLocaleString()}...`);
  const { data: metrics } = await supabase.from("metrics").select("*").eq("user_id", userId).in("type", ["expense", "income"]).gt("value", threshold);
  if (!metrics || metrics.length === 0) {
    return {
...
async function analyzeFinanceData(supabase, userId) {
  ...
  const largeTransactions = expenses.filter((e) => e.value > 2e4);
